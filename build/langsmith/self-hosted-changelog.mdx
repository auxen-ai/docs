...
- Audit Logs: Self-hosted LangSmith now uses go queue by default.
...
- Added an experimental async subagents feature that allowed subagent threads to not block fleet agent executions.
...
- Evaluators created from the playground now correctly track who created them.
- Feedback key filter on the evaluators table now showed a searchable dropdown of existing keys instead of a freeform text input.
- Added instance admin endpoint (`POST /v1/platform/ops/backfills/restart`) to restart backfill jobs. Introduced `INSTANCE_ADMIN_EMAILS` env var (replaced deprecated `USAGE_EXPORT_ADMIN_EMAILS`).
...
- Fixed identity popover missing Slack connection and UI issues.
...
- Fixed an error when creating custom code evaluators from a dataset that uses `perform_eval(run, example)` with two arguments.
...
- Filters in progress on the tracing & threads pages are now preserved when clicking away from the filter popover and reopening it.
...
- Issues agent now requires at least 2 example traces before creating an issue and will look further back in history to find corroborating evidence when only 1 trace is found.
...
- Fixed a bug where rows with no feedback result in the experiment comparison table would show a loading spinner indefinitely instead of showing "No feedback."
...
- Fixed HTTP 500 on the grouped comparison view for datasets where run metadata values are complex objects (dicts/lists) rather than scalars.
- Audit logs now capture dataset download events (`download_dataset`) separately from generic dataset reads. Applies to all export formats (CSV, JSONL, OpenAI, OpenAI FT).
...
- Enforced directional trace tier permissions: users need `projects:increase-trace-tier` to raise retention and `projects:decrease-trace-tier` to lower it.
...
- Self-hosted: enable ABAC and audit-logs.
...
- Introduced new infline filter design.
...
- Issues-board-ao prompt now guides the agent toward fewer, broader subagent calls — screening inline on small scans, consolidating investigation subagents across related categories — reducing cost on large scans.
- GitHub App integration for agent builder — connect GitHub accounts via App installations for repo access.
- Fixed the "Run a skill" browser in cron schedule dialogs now let you actually pick a skill.
...
- Rolled back the issues-board-ao Phase 1b prompt changes from #22845 that caused the agent to undersample traces.
- Fixed LLM evaluators getting stuck with `llm_evaluator.commit_hash_or_tag not found` on save when the stored commit reference is a short commit-hash prefix.
...
- Issues-board-ao Phase 1b screening now uses the new `trace messages` trajectory (input/output preview + step summary) as the first read per trace, only opening the full conversation when trajectory-level signals are ambiguous. Reduces screening cost without changing coverage.
...
- Subagent cards now render at the point they were invoked in the trace view instead of being grouped after all tool calls.
...
- Correcting feedback now updates the chip and the popover.
...
- Fixed a 500 on example creation.
