...
<Accordion title="How the switcher builds its model list" icon="list-search">
    ...
    Models defined in `config.toml` under [`[models.providers.<name>].models`](/oss/python/deepagents/code/configuration#adding-models-to-the-interactive-switcher) bypass the profile filter—they always appear in the switcher regardless of profile metadata. This is the recommended way to add models that are missing from the list.
