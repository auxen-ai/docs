...
<Info>
    **AWS Bedrock and Google VertexAI**
    ...
    Note that certain Anthropic models can also be accessed via AWS Bedrock and Google VertexAI. See the  and [`ChatVertexAI`](/oss/python/integrations/chat/google_vertex_ai) integrations to use Anthropic models via these services.

    For Anthropic models on AWS Bedrock with the same API as `ChatAnthropic`, use  from `langchain-aws`.
</Info>
